...
class _ExchangeReturnHistoryScreenState
  ...
  Future<void> _loadRequests() async {
    ...
    try {
      ...
      setState(() {
        _requests = requests.map((request) => request.toJson()).toList();
        _isLoading = false;
      });
    } catch (e) {
  ...
  Widget _buildRequestList() {
    ...
    for (var request in filteredRequests) {
      ...
      DateTime requestDate = (request['requestDate'] as dynamic).toDate();
      final dateStr = DateFormat('yyyy. M. d').format(requestDate);
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      '요청 ${DateFormat('MM/dd HH:mm').format(requestDate)}',
                      style: GoogleFonts.notoSans(
                        fontSize: 13,
